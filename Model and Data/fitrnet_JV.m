...
set(0, 'DefaultFigurePosition', [0, 0, 10.4352, 7.6623]); % [x, y, width, height]
...
% process jv data to extract key points of interest
[~, ~, ~, ~, FF_values] = process_jv(lhs_filename, jv_filename, processed_lhs_filename, processed_jv_filename, filter);

% Define the file path
file_path = fullfile(folder_name, sprintf('%s %d boxplot.png', boxplot_name, length(FF_values)));
...
if autoHyperparams
    ...
    saveas(gcf, fullfile(folder_name, sprintf('%s hyperopt.png', case_name)))
...
sgtitle(sprintf('(%s) Predicted vs. Observed Values', case_name));
...
% saveas(gcf, fullfile(folder_name, sprintf('%s testing.png', case_name)));
...
axis equal;  % Equal scaling on both axes
hold off;
saveas(gcf, fullfile(folder_name, sprintf('%s.fig', case_name)));
saveas(gcf, fullfile(folder_name, sprintf('%s.png', case_name)));
