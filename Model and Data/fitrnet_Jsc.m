...
title('Predicted vs. Observed Values For Testing (\it{J}_{\rm{sc}}\rm{)}');
...
xlabel('Observed Values');
ylabel('Predicted Values');
title('Predicted vs. Observed Values For Training (\it{J}_{\rm{sc}}\rm{)}');
axis equal;  % Equal scaling on both axes
hold off;
saveas(gcf, fullfile(folder_name, sprintf('%s training (Jsc).png', case_name)));
