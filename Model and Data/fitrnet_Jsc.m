...
autoHyperparams = 1; % Auto Optimize Hyperparameters

filter = 0; % Filter PSCs for FF >= 0.7
...
if ~isfile(file_path) || 1
    ...
    grid on;
    boxplot(FF_values);
    title("Fill Factor Values in Dataset (\it{J}_{\rm{sc}}\rm{)}");
...
if autoHyperparams
    ...
    % using hyperparams from FTF variant
...
figure; 
sgtitle('(FTF \it{J}_{\rm{sc}}\rm{)} Predicted vs. Observed Values');
...
hold off;
% saveas(gcf, fullfile(folder_name, sprintf('%s testing (Jsc).png', case_name)));
